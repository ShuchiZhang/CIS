...
class Generator(nn.Module):
    def __init__(self,
        ...
        if pretrain_model is None:
            ...
            self.pretrain_model, _ = pretrain_LSTMCore(
                train_x=x, vocab_size=len(reverse_vocab))
        else:
    ...
    def forward(self,
        ...
        y, tag_space = self.pretrain_model(x,
                                           hidden,
        ...
        try:
            y_output = y_prob.view(
                -1, y_prob.shape[-1]).multinomial(num_samples=1).view(shape)
        except RuntimeError:
            print('error with multinomial. using argmax instead.')
            y_output = torch.argmax(y_prob.view(-1, y_prob.shape[-1]),
                                    dim=1).view(shape)
        if rewards is None:
    ...
    def generate_LSTMCore(self, start_token, ignored_tokens, batch_size=1):
        ...
        with torch.no_grad():
            ...
            for i in range(SEQ_LENGTH - 1):
                ...
                y_pred, tag_space = self.pretrain_model(
                    x,
                    hidden,
                    sentence_lengths=torch.tensor([1], device=DEVICE).long())
                # random choice based on probability distribution. another possibility would be to take the max.
                y_prob = F.softmax(self.ignoreTokens(tag_space,
                                                     ignored_tokens),
                                   dim=2)
                shape = (y_prob.shape[0], y_prob.shape[1])
...
class GeneratorLoss(nn.Module):
    ...
    def forward(self, prediction, x, rewards):
        ...
        x2 = self.createOneHotDummy(dim=(x1.shape[0],
                                         pred1.shape[1])).scatter_(1, x1, 1)
...
def sanityCheck_generator(model=None, batch_size=1, sample_size=5):
    ...
    if model is None:
        ...
        model.to(DEVICE)
    model, y_prob_all, y_output_all = train_generator(model,
                                                      x,
                                                      reward=None,
