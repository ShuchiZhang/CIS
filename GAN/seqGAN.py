...
def pretrain_generator(x,
    ...
    '''
      ...
      if generator is also DataParallel, when it calls lstmCore it invokes the error message "RuntimeError: all tensors must be on devices[0]"
...
def main(batch_size, num=None):
    ...
    # ignore start token, end token and pad token
    ignored_tokens = [start_token, end_token, pad_token]
    vocab_size = len(vocabulary)

    # start to pretrain generator
    log = openLog()
    ...
    x_gen = generator.generate(start_token=start_token,
                               ignored_tokens=ignored_tokens,
                               batch_size=len(x))
    # start to pretrain discriminator
    log = openLog()
