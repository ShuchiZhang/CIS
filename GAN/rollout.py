...
from config import (SEQ_LENGTH, EMB_SIZE, GEN_HIDDEN_DIM, ROLLOUT_ITER, DEVICE,
                    openLog)
...
class Rollout(nn.Module):
    def __init__(self, generator=None, r_update_rate=0.8, vocab_size=10):
        ...
        if generator is not None:
            ...
            self.hidden2tag = copy.deepcopy(
                generator.pretrain_model.module.hidden2tag)
    ...
    def forward(self, sentence, hidden, given_num, ignored_tokens=None):
        ...
        self.y_prob = self.softmax(
            self.ignoreTokens(self.tag_space, ignored_tokens))
        shape = (self.y_prob.shape[0], self.y_prob.shape[1])
        y_prob_existing_output = self.y_prob.view(
            -1, self.y_prob.shape[-1]).multinomial(num_samples=1).view(shape)
        ...
        for i in range(given_num, SEQ_LENGTH):
            ...
            y_prob_rollout = self.softmax(
                self.ignoreTokens(tag_space_rollout, ignored_tokens))
            x_t = y_prob_rollout.multinomial(num_samples=1)
            self.lstm_out = torch.cat(
                (self.lstm_out, hidden0[0].view(-1, 1, hidden0[0].shape[-1])),
                dim=1)
            self.tag_space = torch.cat(
                (self.tag_space,
                 tag_space_rollout.view(-1, 1, tag_space_rollout.shape[-1])),
                dim=1)
            self.y_prob = torch.cat(
                (self.y_prob,
                 y_prob_rollout.view(-1, 1, y_prob_rollout.shape[-1])),
                dim=1)
    ...
    def update_params(self, generator):
        for p, w in zip(self.lstm.parameters(),
                        generator.pretrain_model.module.lstm.parameters()):
            p.data = self.r_update_rate * p.data + (
                1 - self.r_update_rate) * w.data
        for p, w in zip(
                self.hidden2tag.parameters(),
                generator.pretrain_model.module.hidden2tag.parameters()):
            p.data = self.r_update_rate * p.data + (
                1 - self.r_update_rate) * w.data
    ...
    with torch.no_grad():
        ...
        for i in range(ROLLOUT_ITER):
            ...
            for given_num in range(1, SEQ_LENGTH):
                ...
                tag_scores, rollout_output = rollout(sentence=gen_output,
                                                     hidden=hidden,
                                                     given_num=given_num)
