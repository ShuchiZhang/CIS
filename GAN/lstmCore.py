...
from config import (SEQ_LENGTH, EMB_SIZE, DEVICE, GEN_HIDDEN_DIM,
                    GEN_NUM_EPOCH_PRETRAIN, openLog)
from data_processing import gen_record, read_sampleFile, decode


def init_matrix(shape, stdDev=0.1):
    normalDistr = Normal(torch.tensor([0.0], device=DEVICE),
...
class LSTMCore(nn.Module):
    ...
    def forward(self, sentence, hidden, sentence_lengths=None):
        ...
        if sentence_lengths is None:
            sentence_lengths = torch.LongTensor([sentence.shape[1]] *
                                                len(sentence))
...
def pretrain_LSTMCore(train_x=None,
    ...
    for epoch in range(GEN_NUM_EPOCH_PRETRAIN):
        ...
        while pointer + batch_size <= len(x):
            ...
            x0_length = torch.tensor(
                sentence_lengths[pointer:pointer +
                                 batch_size]).to(device=DEVICE)
            y = torch.cat(
                (x_batch[:, 1:],
                 torch.tensor([end_token] * x_batch.shape[0],
                              device=DEVICE).int().view(x_batch.shape[0], 1)),
                dim=1)
