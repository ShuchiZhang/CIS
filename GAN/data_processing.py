...
def gen_label(num=GENERATE_NUM, target_space=2, fixed_value=None):
    ...
    if fixed_value is None:
        ...
        return torch.randint(low=fixed_value,
                             high=fixed_value + 1,
                             size=(num, ),
                             device=DEVICE).long()
...
# read urls.pkl
def read_sampleFile(file='urls.pkl', pad_token='PAD', num=None):
    # pkl or csv
    ...
    generated_data = [
        int(vocabulary[x]) for y in lineList_all for i, x in enumerate(y)
        if i < SEQ_LENGTH
    ]
